...
public interface IPersonRepository
{
    ...
    Task<(IReadOnlyList<string> followingIds, double ru)> ListFollowingIdsAsync(string personId, CancellationToken ct = default);
...
g.V().has('person','personId',personId).limit(1).valueMap(true)";
    ...
    public async Task<(IReadOnlyList<string> followingIds, double ru)> ListFollowingIdsAsync(string personId, CancellationToken ct = default)
    {
        var script = @"
g.V().has('person','personId',personId).limit(1).
  out('follows').
  values('personId')";
        var bindings = new Dictionary<string, object?> { ["personId"] = personId };

        var rs = await _client.SubmitAsync<string>(script, bindings).ConfigureAwait(false);
        var ids = rs
